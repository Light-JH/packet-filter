...
# 
#
# Jinjuan Hu: Update Internet_IP to correct host address
...
#
# Jinjuan Hu: Update Client_NET_IP to correct host address
...
# Jinjuan Hu: Allow Forwarding from client network to gateway network
#             for TCP with destination port 80 (HTTP traffic)
$IPTABLES -A FORWARD -s $Client_NET_IP_RANGE -d $Internet_IP_RANGE -p tcp --dport 80 -j ACCEPT


# Jinjuan Hu: Allow Forwarding from client network to DNS (8.8.8.8)
#             for ICMP echo requests (ping)
$IPTABLES -A FORWARD -s $Client_NET_IP_RANGE -d 8.8.8.8 -p icmp --icmp-type echo-request -j ACCEPT
...
# Jinjuan Hu: Allow Input for TCP packets to port 80 (HTTP)
#             coming from the client network to the gateway
$IPTABLES -A INPUT -p TCP --dport 80 -s $Client_NET_IP_RANGE -d $WEB_IP_ADDRESS -j ACCEPT

# Jinjuan Hu: Allow Input for TCP packets to port 80 (HTTP)
#             coming from the gateway to the gateway (loopback)
$IPTABLES -A INPUT -p TCP --dport 80 -s 127.0.0.0/8 -d $WEB_IP_ADDRESS -j ACCEPT

# Jinjuan Hu: Allow TCP SYN packets. Necessary for allowing HTTP connections to establish.
$IPTABLES -A INPUT -p TCP --syn -j ACCEPT

# Jinjuan Hu: Allow TCP packets for established/related packets to maintain the connection.
...
#$IPTABLES -A INPUT -d 8.8.8.8 -p icmp -j ACCEPT
...
# Jinjuan Hu: Allow Outgoing TCP packets from port 80 (HTTP Response)
#             going to the client network from the gateway
$IPTABLES -A OUTPUT -p TCP --sport 80 -s $WEB_IP_ADDRESS -d $Client_NET_IP_RANGE -j ACCEPT

# Jinjuan Hu: Allow Outgoing TCP packets from port 80 (HTTP Response)
#             going to the gateway from the gateway (loopback)
$IPTABLES -A OUTPUT -p TCP --sport 80 -s $WEB_IP_ADDRESS -d 127.0.0.0/8 -j ACCEPT

# Jinjuan Hu: Allow Outgoing TCP packets for established/related packets to maintain the connection.
$IPTABLES -A OUTPUT -p TCP -m state --state ESTABLISHED,RELATED -j ACCEPT


# Jinjuan Hu: Allow Outgoing ICMP packets coming from 8.8.8.8
